 ...
 */
package com.it.iddl.idatasource.resource.pool.connection;
...
import com.it.iddl.idatasource.tx.TxUtils;
...
public class CachedConnectionManager {
   ...
   private static final Logger logger = Logger.getLogger(CachedConnectionManager.class);
   ...
   {
      if (specCompliant)
         logger.warn("THE SpecCompliant ATTRIBUTE IS MISNAMED SEE http://jira.jboss.com/jira/browse/JBAS-1662");
           ...
           throws ResourceException
   {
      ...
      {
         if (trace)
            logger.trace("new stack for key: " + Strings.defaultToString(rawKey));
      ...
      {
         if (trace)
            logger.trace("old stack for key: " + Strings.defaultToString(rawKey));
           ...
           throws ResourceException
   {
      ...
      if (trace)
         logger.trace("popped object: " + Strings.defaultToString(oldKey));
      if (specCompliant)
      {
   ...
   void registerConnection(ConnectionCacheListener cm, ConnectionListener cl, Object connection, ConnectionRequestInfo cri)
   {
      ...
      if (trace)
         logger.trace("registering connection from " + cm + ", connection : " + connection + ", key: " + key);
      if (key == null)
   ...
   void unregisterConnection(ConnectionCacheListener cm, Object c)
   {
      ...
      KeyConnectionAssociation key = peekMetaAwareObject();
      if (trace)
         logger.trace("unregistering connection from " + cm + ", object: " + c + ", key: " + key);
      if (key == null)
         return; //not participating properly in this management scheme.
           ...
           throws SystemException
   {
      KeyConnectionAssociation key = peekMetaAwareObject();
      if (trace)
         logger.trace("user tx started, key: " + key);
      if (key == null)
         return; //not participating properly in this management scheme.
   ...
   {
      if (trace)
         logger.trace("unregisterConnectionCacheListener: " + cm);
      synchronized (objectToConnectionManagerMap)
      {
   ...
   private void closeConnection(Object c)
   {
      try
      {
         ...
         {
            if (e != null)
               logger.info("Closing a connection for you.  Please close them yourself: " + c, e);
            else
               logger.info("Closing a connection for you.  Please close them yourself: " + c);
            m.invoke(c, new Object[]{});
         }
         catch (Throwable t)
         {
            logger.info("Throwable trying to close a connection for you, please close it yourself", t);
         }
      }
      catch (NoSuchMethodException nsme)
      {
         logger.info("Could not find a close method on alleged connection objects.  Please close your own connections.");
      }
   }
   ...
   private CloseConnectionSynchronization getCloseConnectionSynchronization(boolean createIfNotFound)
   {
      ...
      {
         logger.debug("Unable to synchronize with transaction", t);
      }
      return null;
   }
