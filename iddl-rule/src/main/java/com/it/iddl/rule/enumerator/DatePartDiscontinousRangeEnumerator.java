...
import com.it.iddl.rule.DateEnumerationParameter;
...
public class DatePartDiscontinousRangeEnumerator extends PartDiscontinousRangeEnumerator {
	...
	protected Comparative changeGreater2GreaterOrEq(Comparative from) {
		if (from.getComparison() == Comparative.GREATER_THAN) {
			...
			return new Comparative(Comparative.GREATER_THAN_OR_EQUAL, new Date(
					gtOrEqDate));
	...
	protected Comparative changeLess2LessOrEq(Comparative to) {
		if (to.getComparison() == Comparative.LESS_THAN) {
			...
			return new Comparative(Comparative.LESS_THAN_OR_EQUAL, new Date(
					lessOrEqDate));
		}
		return to;
	}
