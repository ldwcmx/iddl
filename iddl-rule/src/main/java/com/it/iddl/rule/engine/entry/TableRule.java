...
import com.it.iddl.common.exception.runtime.CantNotIdentifyNumberExcpetion;
import com.it.iddl.common.exception.runtime.IDDLRuntimeException;
...
import com.it.iddl.rule.type.TypeHandlerUtils;
...
//		}
		...
		if (targetTables != null && !targetTables.equals("")) {
			...
			case 2:
				...
				break;
			default:
				throw new UnsupportedOperationException("还不支持的情况");
			}
		}
		return retCol;
	}

	private void getDefaultTables(Collection<String> collection, String tabToken, int step, TableRuleType type) {
		List<Integer> startEnd = new ArrayList<Integer>(); 
		StringBuffer vTab = new StringBuffer();;
		TableNameTypeHandler tHandler = TableNameTypeRegister.getTableNameHandler(type);
		...
		if (curPos < tabToken.length() -1) {
			...
			if (curPos == -1) {
				throw new IDDLRuntimeException("解析defaultTableRange出错" + tabToken);
			}
	...
	private int parseDefaultTables(int cur, String tabToken, List<Integer> startEnd, StringBuffer vTab) {
		...
		try {
			...
			throw new CantNotIdentifyNumberExcpetion(range[0], range[1], e);
		}
		return ed;
	}
