...
import com.it.iddl.parser.sql.objecttree.ORExpressionGroup;
import com.it.iddl.parser.sql.objecttree.RowJepVisitor;
...
public class SkipMaxUtils {
    ...
    public static int getRowNumSkipToInt(Comparative co) {
        ...
        } else if (co.getComparison() == Comparative.GREATER_THAN) {
        	//> 的情况
            return getComparativeToInt(co);
        } else if (co.getComparison() == Comparative.GREATER_THAN_OR_EQUAL) {
        	//>=的情况
...
//        }
   ...
   public static int getRowNumMaxToInt(Comparative co) {
        ...
        if (co instanceof ComparativeAND) {
            ...
            throw new IllegalArgumentException("rownum目前不支持=关系");
        } else if (co.getComparison() == Comparative.LESS_THAN) {
        	 //< 的情况
            return getComparativeToInt(co)-1;
        } else if (co.getComparison() == Comparative.LESS_THAN_OR_EQUAL) {
        	//<=的情况
            return getComparativeToInt(co);
        } 
        return DMLCommon.DEFAULT_SKIP_MAX;
    ...
    public static void nestedBuildRownumGroup(ExpressionGroup source,
		...
		if (targetRownumGroup != null) {
			for (TableName tname : tabNames) {
				//先找表名是个select的。
				if (tname instanceof TableNameSubQueryImpl) {
					Select select = ((TableNameSubQueryImpl) tname)
							.getSubSelect();
