...
import com.it.iddl.parser.ComparativeMapChoicer;
import com.it.iddl.parser.SQLParserResult;
import com.it.iddl.parser.sql.function.Distinct;
...
import com.it.iddl.parser.sql.objecttree.output.handler.impl.PlaceHolderReplaceHandler;
import com.it.iddl.parser.sql.objecttree.output.handler.impl.RangePlaceHandler;
import com.it.iddl.parser.sql.objecttree.traversal.GroupByTraversalAction;
import com.it.iddl.parser.sql.objecttree.traversal.OrderByTraversalAction;
import com.it.iddl.parser.sql.objecttree.traversal.TableNameTraversalAction;
import com.it.iddl.parser.sql.objecttree.traversal.TraversalSQLAction;
import com.it.iddl.parser.sql.objecttree.traversal.TraversalSQLEvent;
import com.it.iddl.parser.sql.objecttree.traversal.TraversalSQLEvent.StatementType;
...
public abstract class DMLCommon implements Statement, SQLParserResult, ComparativeMapChoicer {
	...
	protected List<OrderByElement> orderByElementList = Collections.emptyList();
	protected List<GroupByElement> groupByElementList = Collections.emptyList();
	protected List<InExpressionObject> inObjList = new ArrayList<InExpressionObject>(1);

	protected List<Hint> hintList = new ArrayList<Hint>(1);
	 ...
	 * 
	 * 主要用于解决嵌套查询中xx内定的str2varlist和str2numlist函数。
	 */
	 ...
	 * @return
	 */
	public final Map<String, Comparative> getColumnsMap(List<Object> argumentList, Set<String> partnationSet) {
		Map<String, Comparative> copiedMap = new HashMap<String, Comparative>(partnationSet.size());
		for (String aArgument : partnationSet) {
			 ...
			 * comparative); } } }
			 */
			Comparative comparative = getColumnComparative(argumentList, aArgument);
			if (comparative != null) {
	...
	public void registerTraversalActionAndGet() {
		...
		orderByElementList = orderby.getOrderByElementList();
		groupByElementList = groupby.getGroupByElementList();
	}
	...
	protected void initAliasAndComparableMap(
		...
		for (TableName name : tbNames) {
			if (name instanceof TableNameSubQueryImpl) {

				if (hasOneSubSelect) {
					throw new IllegalArgumentException("同级sql不允许出现多个子sql");
				}
				hasOneSubSelect = true;
				TableNameSubQueryImpl subSql = (TableNameSubQueryImpl) name;
	...
	public void traversalSQL(List<TraversalSQLAction> traversalSQLActions,
		...
		for (TableName tbName : tbNames) {
			if (tbName instanceof TableNameSubQueryImpl) {
				Select select = ((TableNameSubQueryImpl) tbName).getSubSelect();
	...
	protected int getRangeOrMax(List<Object> param) {
		...
		for (TableName tbName : tbNames) {
			...
			if (tbName instanceof TableNameSubQueryImpl) {
				temp = ((TableNameSubQueryImpl) tbName).getSubSelect()
						.getRangeOrMax(param);
	...
	public int getSkip(List<Object> param) {
		...
		for (TableName tbName : tbNames) {
			if (tbName instanceof TableNameSubQueryImpl) {
				temp = ((TableNameSubQueryImpl) tbName).getSubSelect().getSkip(
						param);
				if (temp > skip) {
					skip = temp;
				}
	...
	public abstract List<OrderByElement> nestGetOrderByList();

	public abstract List<GroupByElement> nestGetGroupByList();
	...
	public List<String> getDistinctColumn() {
		if (null != distinct) {
			if (null != distinct.getColumns()) {
				return distinct.getColumns().getColumnList2String();
			}
		}
			return null;
	}
