...
public class Select extends DMLCommon implements SubQueryValue {
	...
	public TableNameFunctionImpl getTableNameFunction() {
		TableNameFunctionImpl snap = null;
		for (TableName name : tbNames) {
			if (name instanceof TableNameFunctionImpl) {
				TableNameFunctionImpl tbNameFunction = (TableNameFunctionImpl) name;
				if (snap != null) {
	...
	public List<GroupByElement> nestGetGroupByList() {
		return where.getGroupByColumnList();
	}
	...
	public Comparable<?> getValue(List<Object> args) {
		TableNameFunctionImpl tabFunc = getTableNameFunction();
		Function func = null;
		if (tabFunc != null) {
